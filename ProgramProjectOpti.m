...
%Remarque : Cette deernière ligne n'a pas d'utilité mais sert seulement à
%améliorer la lisibilité des réponses. Elle reprend exactement le stock
%précédent dans son stock et les productions sont nulles. C'est une semaine
%fictive, tout comme la prmière.
...
beq = zeros(1,d.T);
beq(1) = d.demande(1);
beq(d.T) = 0; %Pour que le stock revienne à stock_initial
for i = 2:d.T-1
...
Aeq(1,3) = 1;
%Indique que le stock de la dernière ligne est simplement celui d'avant
...
for i = 1:length(f)
    ...
    if i == ((length(f))/5-2)*5+3 %Oblige le stock à revenir au stock initial à la fin de la planification
        ub(i) = d.stock_initial;
        lb(i) = d.stock_initial;
    end
    if i == ((length(f))/5-2)*5+4 %On peut pas rendre en retard la dernière semaine !
        ub(i) = 0;
    end
end
options = optimoptions('linprog', 'Algorithm', 'simplex');%Pour qu'il nous donne bien une sol entière s'il y en a une
